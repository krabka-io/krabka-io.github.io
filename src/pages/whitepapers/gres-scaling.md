...
Krabka Gres is a pure-Rust, Postgres-compatible compute tier that stores all durable state on the Krabka substrate: per-range Kafka write-ahead-log topics and object-store checkpoints.
...
The engine is Postgres-compatible at the wire and SQL-semantics level (pgwire v3, extended query protocol, PostgreSQL-faithful MVCC), not at the page or physical-WAL level; tenants needing C extensions or byte-level fidelity belong to Krabka's separate real-Postgres tier.
...
A plain, unadorned `CREATE TABLE` from a stock Postgres application therefore scales without the application ever learning Krabka syntax, and unconverted tables remain byte-identical to the single-range baseline.
