//! WebAssembly bindings that drive Krabka's deterministic `KRaft` consensus
//! engine in the browser.
//!
//! The website page `/docs/playground` simulates consensus in the browser. The
//! deterministic core compiles to WASM. The page can then inject partitions,
//! drop, reorder, and duplicate messages, and watch a cluster elect a leader,
//! lose it, and recover, live and with no backend. This crate is the seam. It
//! wraps [`krabka_kraft_core::sim::Sim`] and exposes it to JavaScript as a
//! [`Playground`] handle. `Sim` is the same pure, sans-IO multi-node simulator
//! that the broker integration tests drive.
//!
